...
class pageClass { 
	...
	var $topmenu = array();
	var $recipeCategoriesLeftMenu = array();
	...
	function makeMenuItem($menu, $label, $page, $class = "") {
		...
		if ($menu == "recipeCategoriesLeft") {
			$this->recipeCategoriesLeftMenu[$label] = array();
			$this->recipeCategoriesLeftMenu[$label]['value'] = $this->makeNavURL($page);
			if ($class != "") { $this->recipeCategoriesLeftMenu[$label]['class'] = $class; }
		}
	...
	function printSidebar($tagId, $tagClass = "") {		
		...
		if ($this->templateVars['pCLm_active'] == ACTIVE) {
			$sidebarHTML .= "
					<li class=\"title\">Recipe Categories</li>";
			
			foreach ($this->recipeCategoriesLeftMenu as $text => $page) {
				$sidebarHTML .= "
					<li><a href=\"" . $this->recipeCategoriesLeftMenu[$text]['value'] . "\"><span>" . $text . "</span></a></li>";
			}
		}
